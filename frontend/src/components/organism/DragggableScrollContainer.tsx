  ...
  return (
    <>
      <div
        ...
        className="flex overflow-x-scroll justify-center gap-5 px-5 py-5 cursor-grab active:cursor-grabbing select-none scrollbar-hide scroll-smooth"
      >
        {children}
      </div>
        ...
        {Array.from({ length: pageCount }).map((_, index) => (
          <button
            ...
            className={`w-3 h-3 rounded-full transition-all ${
              index === currentPage ? "bg-green-800 scale-130" : "bg-green-primary"
            }`}
          />
        ))}
      </div>
